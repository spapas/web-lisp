...
(test dynamic-query-builder-tests
  "Test the dynamic query builder"
  (let ((qq (web-lisp-db:dynamic-query :fields (u.id u.username u.email)
                                      :tables ((user u))
                                      :where (:and (u.is_active = 1)
                                                   (:or (u.username = "foo") (u.username = "root"))))))

    (is (equal qq "SELECT u.id, u.username, u.email FROM user u WHERE (u.is_active = 1 AND (u.username = 'foo' OR u.username = 'root'))"))))  
